...
days	int	The number of days back from the current date to include in the results ( publish date ). Available only when using the "news" topic.	7	
time_range	str	The time range back from the current date ( publish date ). Accepted values include "day", "week", "month", "year" or shorthand values "d", "w", "m", "y".	—	
start_date	str	Will return all results after the specified start date ( publish date ). Required to be written in the format YYYY-MM-DD	—	
end_date	str	Will return all results before the specified end date ( publish date ). Required to be written in the format YYYY-MM-DD.	—	
